...
int FCSstepFill(QVector<double> &tau, QVector<double> &corrDonor, QVector<double> &corrAcceptor, QVector<double> &crossCorr, QVector<double> &corrSum, const QVector<double> &arrivalTimeDonor,const QVector<double> &arrivalTimeAcceptor, const QVector<double> markerTimeDexc,  const QVector<double> markerTimeAexc, const QVector<BurstDual> &burstVectorDual, const FCSparams_t &FCSparams )
{
    ...
    for(int i=0; i<burstVectorDual.size(); ++i) { // concatenate all bursts
        ...
        while(i<histDonor->n) {
            while((histDonor->range[i]<=(Tend_donor+n*Tend_pause2)) && i<histDonor->n) { //while(histOsc->range[i]<T1) histOsc->bin[i++]++;
                histDonor->bin[i]=1;
                histAcceptor->bin[i]=0;
                i++;
            }
            while((histDonor->range[i]<=(Tend_pause1+n*Tend_pause2)) && i<histDonor->n) {
                histDonor->bin[i]=0;
                histAcceptor->bin[i]=0;
                i++;
            }
            while((histDonor->range[i]<=(Tend_acceptor+n*Tend_pause2)) && i<histDonor->n) {
                histDonor->bin[i]=0;
                histAcceptor->bin[i]=1;
                i++;
            }
            while((histDonor->range[i]<=(Tend_pause2+n*Tend_pause2)) && i<histDonor->n) {
                histDonor->bin[i]=0;
                histAcceptor->bin[i]=0;
                i++;
            }
